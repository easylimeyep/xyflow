...
#### Scenario: Shortcut evaluator path receives visible clearance

- **WHEN** the large ELK workflow example is laid out
- **THEN** shortcut evaluator connections from an evaluator output directly to a result node SHALL have visible clearance from unrelated intermediate node bodies
