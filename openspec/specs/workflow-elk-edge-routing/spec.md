...
#### Scenario: Evaluator shortcut skips intermediate true-path nodes

- **WHEN** an ELK-backed workflow contains an evaluator edge that connects directly to a downstream result while a sibling evaluator path passes through intermediate nodes
