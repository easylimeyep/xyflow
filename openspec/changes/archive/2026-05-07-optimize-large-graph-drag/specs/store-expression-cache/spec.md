...
#### Scenario: Cache is not projected during position-only drag
- **WHEN** a node drag emits position-only updates
- **THEN** the store SHALL NOT rebuild expression catalog cache entries for those updates
- **AND** the current cache SHALL remain available to expression inputs
