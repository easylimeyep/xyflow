...
import {
  DEFAULT_EXAMPLE_TAB,
  EXAMPLE_TAB_SEARCH_PARAM,
  isExampleTab,
} from "@/app/components/workflow-examples/example-tabs"
...
export function ExamplesPage() {
  ...
  const tabParam = searchParams.get(EXAMPLE_TAB_SEARCH_PARAM)
  const activeTab = isExampleTab(tabParam) ? tabParam : DEFAULT_EXAMPLE_TAB

  const handleTabChange = useCallback(
    (value: string) => {
      const params = new URLSearchParams(searchParams.toString())

      if (value === DEFAULT_EXAMPLE_TAB) {
        params.delete(EXAMPLE_TAB_SEARCH_PARAM)
      } else {
        params.set(EXAMPLE_TAB_SEARCH_PARAM, value)
      }
