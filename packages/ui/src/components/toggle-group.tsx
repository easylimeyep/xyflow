...
import { toggleVariants } from "@flow/ui/components/toggle"
...
function ToggleGroup({
  ...
  return (
    <ToggleGroupPrimitive
      ...
      className={cn(
        "group/toggle-group flex w-fit flex-row items-center gap-(--gap) rounded-md data-[size=sm]:rounded-[min(var(--radius-md),8px)] data-vertical:flex-col data-vertical:items-stretch",
        className
      )}
      {...props}
    >
